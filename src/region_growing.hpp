...
ushort rar_seeds[] = {0, 0, 511, 511};
ushort car_seeds[] = {0, 511, 0, 511};

inline bool stage1_lambda(img* vol, img* delta, int z, int y, int x, ushort shade) {
...
inline bool stage2_lambda(img* vol, img* delta, int z, int y, int x, ushort shade) {
...
static bool locate_seed(img* vol, ushort match,
	vector<ushort>& rar, vector<ushort>& car, vector<ushort>& zar) {

	for (ushort _i=128; _i < 384; ++_i) {
		for (ushort _j=128; _j < 384; ++_j) {
...
void binary_flagged_3d_region_growing(img* volume, ushort shade) {
	...
	while (locate_seed(volume, shade, rar, car, zar)) {
		visited = 1;
		visiting = 0;
		flag++;
